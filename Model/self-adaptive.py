...
def init_params(options):
    ...
    params = param_init_lstm(options, params, prefix='lstm_1', in_dim=options['dim_proj'], out_dim=options['dim_hidden'])

    params = param_init_lstm(options, params, prefix='lstm_2', in_dim=options['dim_hidden'],
                                  out_dim=options['dim_hidden'])

    params = param_init_gate(options, params, prefix='gate', dim=options['dim_hidden'])
    # classifier
...
def build_model(tparams, options):
    ...
    # proj = lstm(tparams, emb, options, mask=mask, prefix='lstm_1', in_dim=options['dim_proj'],
    #             out_dim=options['dim_hidden'])
    proj = gate(tparams, emb, options, prefix='gate')
...
if __name__ == '__main__':
    train_err, valid_err, test_err = train(
        dataset='5Labelquchong_300_40000_glove.6B',
        dim_proj=300,
        dim_hidden=64,
        max_epochs=80,
        use_dropout=True,
