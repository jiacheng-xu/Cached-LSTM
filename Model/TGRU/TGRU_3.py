...
def init_params(options):
    ...
    # classifier
    # params['W'] = ortho_weight(3*options['dim_hidden'], options['dim_hidden'])
...
def build_model(tparams, options):
    ...
    proj_11 = conv_naive(tparams, proj[:-1], options, prefix='conv_11', mask=mask[:-2], out_dim=options['dim_hidden'])
    proj_12 = conv_naive(tparams, proj[1:], options, prefix='conv_12', mask=mask[1:-1], out_dim=options['dim_hidden'])
